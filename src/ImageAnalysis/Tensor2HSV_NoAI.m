function [imgHSV] = Tensor2HSV_NoAI(img,ST,HueThetaZero,HueThetaPi)
% Create an image where ST information is encoded in the HSV space.
% 
%      [imgHSV] = Tensor2HSV_NoAI(img,ST,HueThetaZero,HueThetaPi)
%
% Creates an image where tensor orientation encodes Hue and image intensity
% encodes Value. Saturation is set to 1 for all pixels.
%
% INPUTS
%
...
%               intensity respectively encode H and V. Image is fully
%               saturated.
%
%
...
% Check for errors
if(nargin~=4)
   error('Tensor2HSV_NoAI(): wrong number of input arguments.'); 
end

if( (size(img,3)~=1)||(isinteger(img)~=1))
    error('Tensor2HSV_NoAI(): input img must be a integer, grayscale image - use rgb2gray().');
end

if( (isnumeric(HueThetaZero)==0)||(isscalar(HueThetaZero)==0)||(isnumeric(HueThetaPi)==0)||(isscalar(HueThetaPi)==0) )
    error('Tensor2HSV_NoAI(): HueThetaZero and HueThetaPi must be numeric scalars.');
end

if((HueThetaZero<0)||(HueThetaZero>=1)||(HueThetaPi<=0)||(HueThetaPi>1)||(HueThetaZero>=HueThetaPi))
    error('Tensor2HSV_NoAI(): HueThetaZero must range in [0; 1) and HueThetaPi in (0;1] with HueThetaPi>HueThetaZero.');
end

% Map Tensor Orientation and image intensity as H and V
